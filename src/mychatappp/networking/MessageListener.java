...
public class MessageListener extends Thread {
    ...
    int port, key=0, triggerForReceivingFile = 0;

    WritableGUI gui;
    String FileName = "";

    public  MessageListener(WritableGUI gui, int port) { //CK  public MessageListener(WritableGUI gui, int port, int key)
        this.port = port;
        this.gui = gui;
        //this.key = key;
        try {
                    ...
                    if (line != null) {
                        ...
                        if (triggerForReceivingFile == 0 && line != "") {
                            ...
                            for (int i = 0; i < ch.length; i++) {
                                ch[i] = (char) (ch[i]- key);             //CK (ch[i] - key)
